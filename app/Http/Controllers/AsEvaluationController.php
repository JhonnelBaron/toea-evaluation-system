...
class AsEvaluationController extends Controller
{
     ...
     public function store(Request $request)
    {
        ...
        // Calculate filled fields count and progress percentage
        $filledFieldsCount = collect($evaluationData)->filter(function ($value, $key) {
            return $value !== null && !str_ends_with($key, '_remarks');
        })->count();

        $totalFields = count($evaluationData) / 2; // Since each field has a corresponding _remarks field
        ...
        $evaluationData['overall_total_score'] = collect($evaluationData)->filter(function ($value, $key) {
            return $value !== null && !str_ends_with($key, '_remarks');
        })->sum();
